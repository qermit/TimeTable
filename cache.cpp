...
QSqlDatabase Cache::m_db;
Cache* Cache::m_cacheInstance = 0;
QString Cache::m_localDBPath;

Cache::Cache(void)
{
}

Cache::Cache(const QString& localDBPath)
{
	m_localDBPath = localDBPath;

	m_db = QSqlDatabase::addDatabase("QSQLITE", "timetable");
	m_db.setDatabaseName(localDBPath + QDir::separator() + "timetable.db");

	if(!QFile::exists(localDBPath + QDir::separator() + "timetable.db")) // Database does not exist yet
	{
